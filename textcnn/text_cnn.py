...
class TextCNN(object):
    ...
    def __init__(self, word2vec, seq_len, num_classes, vocab_size,
        ...
        self.input_y_one_hot = tf.one_hot(self.input_y, depth=num_classes, name='input_y_one_hot')
        ...
        with tf.device(device), tf.name_scope("embedding"):
            if word2vec is None:
                ...
                self.W = tf.get_variable("word_embeddings", initializer=word2vec.astype(np.float32))
        ...
        with tf.name_scope("loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.input_y_one_hot)
